...
% \emcomment{Mention that + is a function; the syntax is also convenient since it easily accommodates a variable number of %arguments} \tscomment{Resolved?}
	...
	An example of a function that multiplies a given number by two looks like:
	 \begin{verbatim}
	(defn multiply-two [n] (* n 2))
	\end{verbatim}
	In this example:
	\begin{itemize}
	\item {\tt defn} indicates the declaration of a function.
	\item {\tt multiply-2} is the function name.
	\item \texttt{[n]} is the argument list.
	\item {\tt (* n 2)} is the expression that multiplies n by 2.
	\end{itemize}
	...
	changing values of a variable. One huge benefit of using immutable data types is minimizing side effects, which means 
